...
- **Updated the sync engine's text-processing library** — `golang.org/x/text` 0.37.0 → 0.39.0 fixes a publicly disclosed denial-of-service issue ([GO-2026-5970](https://pkg.go.dev/vuln/GO-2026-5970), infinite loop on invalid input) in Unicode normalization code that the embedded sync engine does reach; flagged by the merge-blocking govulncheck gate on this release's PR. `golang.org/x/sys` and `golang.org/x/tools` moved along as transitive requirements.
...
- **Future diagnostics-namespace visibility and retention are disclosed before runtime work** — the policy and dormant consent copy explain that `VaultSync Diagnostics` would be visible in Obsidian, Files, configured peers, backups, versions, remote history, conflict copies, and tombstones; retained copies may outlive live cleanup or TTL. The namespace foundation stores no note content, user-derived filename, local display label, Syncthing credential, or Cloud Relay contract data, and no current product flow creates it.
