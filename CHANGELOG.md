...
- **Cloud Relay activates itself** — after you subscribe, VaultSync registers your devices with the relay automatically, and the server helper (`vaultsync-notify`) now confirms activation on its own: the moment it starts it sends one wake-up, and the app flips to **Cloud Relay active** with no extra step or change needed. A reactivation card reaches subscribers whose server has never woken their iPhone, and the Cloud Relay screen shows honest live states — **not active yet**, **active**, or **went quiet** — instead of guessing. Localized in English, German, Spanish, and Simplified Chinese.
