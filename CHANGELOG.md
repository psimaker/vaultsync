...
- **Finishing the welcome setup no longer shows a "sync already running" error** ([#61](https://github.com/psimaker/vaultsync/issues/61)) — completing setup while a background wake-up had already started the sync engine surfaced a confusing error in the very first moment of use. The app now attaches to the running engine instead, exactly like a normal foreground return.
